"""KittyViewer — archived rendering backend using the Kitty graphics protocol.

This viewer sends rendered pixel buffers directly to Kitty-compatible terminals
via the graphics protocol (temp file + zlib). It was the default viewer before
the project switched to braille-based text rendering exclusively.

To restore: copy this file back into moltui/, wire up the imports in app.py,
and re-add the --text flag logic in run(). The helper functions
(_get_terminal_pixel_size, _get_temp_path, _kitty_send, _kitty_delete) are
self-contained and included below.
"""
